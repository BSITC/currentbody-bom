...
class Assembly_model extends CI_Model
{
	...
	public function saveAssembly($datas, $assemblyId =""){
					...
					foreach ($postStockTransferArray as $key => $postStockTransfers) {
						if($postStockTransfers){
							...
							$saveArray = array(
								...
								'assignToUserId' 	=> @$datas['assignToUser'],
		...
		$assignToUser = $datas['assignToUser'];
	...
	public function validateDefaultCurrency($config = array()){
		...
		if($currencyResponses){
			...
			if($currencyDatas){
				$defaultCurrencyId = "";
				$defaultCurrencyId = reset($this->brightpearl->getAccountInfo())['configuration']['defaultCurrencyId'];
	...
	public function assemblyDataById($createdId, $assignToUser, $toAdmin=''){
		...
		if($createdId){
			...
			if($allproducts){
				...
				$printHtml = '<div> Dear '.ucfirst($userData['firstname']). ' ' . ucfirst($userData['lastname']).',<br><p></p>Please note that you have been assigned a Work in Progress Assembly with details below.<br><br>';
				if($toAdmin){
				...
				if($sendToEmail && $printHtml){
					$subject = "WIP Assembly Assigned Notification ".date("Y-m-d");
					if($toAdmin){
    ...
    public function getProduct()
    {
        ...
        $displayProRowHeader = array('id', 'autoAssembly', 'username','orderId','createdId','warehouse', 'productId', 'sku', 'name', 'costingMethod','status', 'created');
        ...
        $datas = $query->select('id,productId,createdId,receipId,sku,created,name,status, autoAssembly,username,warehouse,isOrderAssembly,costingMethod,isImportAssembly,orderId')->limit($limit, $start)->get('product_assembly')->result_array();
        foreach ($datas as $data) {
