...
foreach ($this->accountDetails as $account1Id => $accountDetails) {
	...
	$orderDatas = $this->ci->db->get_where('sales_credit_order',array('isPaymentCreated' => '0','sendPaymentTo' => 'brightpearl'))->result_array();
	...
	foreach($orderDatas as $orderData){
		...
		$customerPaymentRequest = array(
			...
			"amountPaid"        => $amount,
			"paymentDate"       => date('c'),
			"journalRef"        => "Sales Credit for order : " . $orderData['orderId'],
		);
		...
		$this->ci->db->where(array('orderId' => $orderData['orderId']))->update('sales_credit_order',array('createdRowData' => json_encode($createdRowData)));
