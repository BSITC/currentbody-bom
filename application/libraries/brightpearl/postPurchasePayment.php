...
foreach ($this->accountDetails as $account1Id => $accountDetails){
	...
	$orderDatas = $this->ci->db->get_where('purchase_order',array('isPaymentCreated' => '0','sendPaymentTo' => 'brightpearl'))->result_array();
	...
	foreach($orderDatas as $orderData){
		...
		if($amount > 0 ){
			$customerPaymentRequest = array(
				...
				"amountPaid"        => $amount,
				"paymentDate"       => date('c'),
				"journalRef"        => "Purchase receipt for order : " . $orderData['orderId'],
			);
			...
			$this->ci->db->where(array('orderId' => $orderData['orderId']))->update('purchase_order',array('createdRowData' => json_encode($createdRowData)));
			if(!isset($customerPaymentRequestRes['errors'])){
				...
				$this->ci->db->where(array('orderId' => $orderData['orderId']))->update('purchase_order',$updateArray); 
			}
		}
	}	
}
