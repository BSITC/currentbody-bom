...
foreach ($this->accountDetails as $account1Id => $accountDetails) {
	...
	$paymentMathodMappingsTemps = $this->ci->db->get_where('mapping_payment',array('account1Id' => $account1Id))->result_array();
	...
	$orderDatas = $this->ci->db->get_where('sales_order',array('isPaymentCreated' => '0','sendPaymentTo' => 'brightpearl'))->result_array();
	...
	foreach($orderDatas as $orderData){
		...
		if($amount > 0 ){
			$customerPaymentRequest = array(
				...
				"amountPaid"        => $amount,
				"paymentDate"       => date('c'),
				"journalRef"        => "Sales receipt for order : " . $orderRowData['reference'],
			);
			...
			$this->ci->db->where(array('orderId' => $orderData['orderId']))->update('sales_order',array('createdRowData' => json_encode($createdRowData)));
			if(!isset($customerPaymentRequestRes['errors'])){
				...
				$this->ci->db->where(array('orderId' => $orderData['orderId']))->update('sales_order',$updateArray);  
			}
		}
	}	
}
